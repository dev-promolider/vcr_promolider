<template>
  <div class="tw-mb-0">
    ...
    <header class="tw-sticky tw-top-4 tw-z-30 tw-flex tw-items-center tw-bg-white tw-border tw-border-[#E5E3DC] tw-rounded-[18px] tw-shadow-[0_4px_16px_-4px_rgba(28,25,23,0.06)] tw-h-[64px] tw-px-4 tw-mx-4 tw-mb-4">
      ...
      <button class="tw-block md:tw-hidden tw-text-[var(--primary-color)] tw-p-2 hover:tw-bg-gray-100 tw-rounded-full tw-mr-2 tw-transition-colors" @click="changeDrawer">
      ...
      <div v-if="points >= 0" class="tw-hidden md:tw-inline-flex tw-items-center tw-gap-[6px] tw-bg-[var(--primary-color)]/10 tw-border tw-border-[var(--primary-color)]/25 tw-text-[var(--primary-color)] tw-font-outfit tw-font-bold tw-text-[0.85rem] tw-px-4 tw-py-1.5 tw-rounded-full tw-mx-2 hover:tw-bg-[var(--primary-color)]/20 hover:tw--translate-y-[1px] tw-transition-all">
      ...
      <div v-if="courseSelected" class="tw-relative tw-group tw-mx-1">
        <button @click="showProgressModal = true" class="tw-relative tw-w-[45px] tw-h-[45px] tw-flex tw-items-center tw-justify-center tw-rounded-full hover:tw-bg-gray-100 tw-transition-colors">
          <svg class="tw-w-full tw-h-full tw--rotate-90" viewBox="0 0 36 36">
            ...
            <path class="tw-text-[var(--primary-color)]" stroke-dasharray="100, 100" :stroke-dashoffset="100 - currentCourseProgress" stroke-width="3" stroke-linecap="round" stroke="currentColor" fill="none" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
          </svg>
          <div class="tw-absolute tw-inset-0 tw-flex tw-flex-col tw-items-center tw-justify-center">
            <span class="tw-text-[var(--primary-color)] tw-text-[10px] tw-font-bold tw--mt-1">{{ currentCourseProgress }}%</span>
            <svg class="tw-w-3 tw-h-3 tw-text-[var(--primary-color)] tw--mt-[2px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 3v4M19 3v4M5 7h14a2 2 0 012 2v2a6 6 0 01-12 0v-2M12 17v4m-4 0h8M6.5 7h11"></path></svg>
          </div>
        </button>
        <!-- Tooltip nativo -->
      ...
      <div v-if="examDaily" class="tw-relative tw-group tw-mx-1">
        <button @click="isQuestionModalOpen = true" class="tw-w-10 tw-h-10 tw-flex tw-items-center tw-justify-center tw-rounded-full hover:tw-bg-gray-100 tw-transition-colors">
          <svg class="tw-w-5 tw-h-5 tw-text-[var(--primary-color)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
        </button>
        <!-- Tooltip nativo -->
        <div class="tw-absolute tw-top-full tw-left-1/2 tw--translate-x-1/2 tw-mt-2 tw-hidden group-hover:tw-block tw-bg-gray-900 tw-text-white tw-text-xs tw-font-medium tw-px-2 tw-py-1 tw-rounded tw-whitespace-nowrap tw-z-50">
      ...
      <div class="tw-relative tw-mx-1 tw-z-50">
        ...
        <div v-if="isNotifMenuOpen" class="tw-absolute tw-right-0 tw-mt-2 tw-w-80 tw-bg-white dark:tw-bg-[#161e2e] tw-rounded-xl tw-shadow-lg tw-border tw-border-gray-100 dark:tw-border-[rgba(255,255,255,0.08)] tw-overflow-hidden tw-animate-fade-in-up">
          ...
          <div v-if="items.length > 0" class="tw-p-2 tw-border-t tw-border-gray-100 dark:tw-border-[rgba(255,255,255,0.08)]">
            <button class="tw-w-full tw-bg-[var(--primary-color)] hover:tw-bg-[var(--primary-hover)] tw-text-white tw-font-bold tw-text-sm tw-py-2 tw-rounded-lg tw-transition-colors">Ver todo</button>
          </div>
        </div>
      </div>
      ...
      <div class="tw-flex tw-items-center tw-ml-2 tw-z-50">
        ...
        <div class="tw-relative">
          <button @click="isProfileMenuOpen = !isProfileMenuOpen; isNotifMenuOpen = false" class="tw-w-10 tw-h-10 tw-rounded-full tw-overflow-hidden tw-bg-[var(--primary-color)]/10 tw-border-2 tw-border-[var(--primary-color)]/30 hover:tw-scale-105 hover:tw-shadow-[0_0_0_3px_rgba(16,185,129,0.2)] tw-transition-all tw-flex tw-items-center tw-justify-center">
            <img v-if="profileImg" :src="profileImg" class="tw-w-full tw-h-full tw-object-cover" />
            <svg v-else class="tw-w-6 tw-h-6 tw-text-[var(--primary-color)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
          </button>
